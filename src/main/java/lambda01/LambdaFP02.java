...
public class LambdaFP02 {
    ...
    public static void printElements(List<Integer> list){
        ...
        list.stream().distinct().forEach(UtilsClass::printInTheSameLineWithspace);
    ...
    public static void printEvenElementsFunctional(List<Integer> list){
        ...
        list.stream().distinct().filter(UtilsClass::checkToBeEven).forEach(UtilsClass::printInTheSameLineWithspace);


    }
    //3) Create a method to print the square of odd list ele.s on the console in the same line with a space
    //between two consecutive ele.s (Functional Method Reference)

    public static void printSquareOfOddElements(List<Integer> list){

        list.stream().distinct().filter(UtilsClass::checkToBeOdd).map(UtilsClass::getSquare).forEach(UtilsClass::printInTheSameLineWithspace);
    ...
    public static void printCubeOfDistinctOddElements(List<Integer> list){
        ...
        list.stream().distinct().filter(UtilsClass::checkToBeOdd).map(UtilsClass::getCube).forEach(UtilsClass::printInTheSameLineWithspace);
